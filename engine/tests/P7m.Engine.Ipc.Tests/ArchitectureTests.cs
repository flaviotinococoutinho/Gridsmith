...
{
    private static string[] P7mReferencesOf(Assembly assembly) =>
        assembly.GetReferencedAssemblies()
            .Select(a => a.Name ?? "")
            .Where(name => name.StartsWith("P7m.", StringComparison.Ordinal))
            .OrderBy(name => name)
            .ToArray();

    [Fact]
    public void E1_Core_nao_depende_de_nenhuma_outra_camada_P7m()
    {
        // O núcleo DOD é a base: portável para qualquer host, sem IPC/gráficos.
        var core = typeof(SkeletonStore).Assembly;
        Assert.Empty(P7mReferencesOf(core));
    }
    ...
    public void E2_Ipc_e_um_plano_de_controle_independente_do_dominio()
    {
        ...
        Assert.Empty(P7mReferencesOf(ipc));
    }
    ...
    public void E3_Graphics_so_conhece_o_Core()
    {
        ...
        Assert.Equal(new[] { "P7m.Engine.Core" }, P7mReferencesOf(graphics));
    }
    ...
    public void E4_Runtime_orquestra_Core_e_Ipc_mas_nao_Graphics()
    {
        ...
        Assert.Equal(new[] { "P7m.Engine.Core", "P7m.Engine.Ipc" }, P7mReferencesOf(runtime));
    }
