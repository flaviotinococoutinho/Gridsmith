...
| Formato da versão | Inteiro — `BLUEPRINT_DOCUMENT_VERSION = 6`; documento sem `schemaVersion` é tratado como versão `0` |
...
| Teste | `middleware/test/blueprint-migration.test.ts` (um teste nomeado por ramo da 2 → 3 + round-trip do corpus, que inclui as fixtures v4 e v5 congeladas) + `tileset-canonical.test.ts` (migração 4 → 5, inversos e o strip da contaminação v5 no fingerprint v2) + `entity-sprite.test.ts` (migração 5 → 6, a recusa de sprite fora do atlas e a ordem tileset → entitydef no replay) + `grid-coordinates.test.ts` + `project-session-manager.test.ts` |
