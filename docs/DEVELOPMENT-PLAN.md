...
| B6 (parte) | ✅ **O SPRITE existe no domínio** (documento v6): `EntityDefinition.sprite` é o par (`tilesetId`, `tileId`) do MESMO atlas dos tilemaps — nada de um segundo caminho de arte —, o tileset precisa existir e o índice estar dentro dele, e o replay define o tileset ANTES da definição que o referencia. **Falta o FIO**: o sprite não chega à engine, então a projeção sai `skipped` dizendo exatamente isso, e o ator segue desenhado como círculo. Transform/animação/colisão continuam fora | os parâmetros de spawn na engine são `(EntityId, ArchetypeId, Position)`; `ausente: sprite @ engine/src/Gridsmith.Engine.Core/Actors` | B6(ii) o fio + paridade, B6(iii) o canvas |
